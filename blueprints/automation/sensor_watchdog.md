...
Starting in v1.0.6-beta the block window reads from the built-in HA Uptime integration directly. Add the integration via Settings > Devices & Services > Add Integration > Uptime; it produces a sensor (`sensor.uptime` by default). The modern config-flow Uptime integration produces a timestamp-mode sensor whose state is an ISO8601 datetime marking the moment HA last started. The blueprint reads that timestamp and computes elapsed seconds via `(now() - sensor_value).total_seconds()`, then compares against the configured `ha_start_block_seconds`. While elapsed seconds is below the threshold, the stale, unavailable, and scheduled recovery branches exit silently. Once it crosses the threshold, the gate opens and the blueprint operates normally.

A subtle bug rode along from v1.0.6-beta through v1.0.8-beta and was fixed in v1.0.9-beta: the original gate template used `states(uptime_sensor) | float(99999)` and a `unit_of_measurement` lookup to handle numeric duration mode. The modern config-flow Uptime integration produces only timestamp mode (no numeric option), so `float()` on the ISO8601 datetime failed and fell back to the sentinel `99999`. The comparison `99999 < ha_start_block_seconds` was always false, so the gate was silently inert across all v1.0.6-beta through v1.0.8-beta deployments. The post-restart branch was unaffected because it uses `ha_start_block_seconds` as a `delay:` directly, but the stale, unavailable, and scheduled branches lost their boot-window protection. v1.0.9-beta replaced the template with the `as_datetime()` + `now()` form above; the gate now engages correctly. The bug went undetected partly because v1.0.7-beta's switch from `mode: queued` to `mode: single` absorbed the false-positive triggers the gate would otherwise have suppressed.
...
**Verified in production (v1.0.9-beta):** pending. The author's home test run continues on three FP2 instances (one in responsive mode with the active window enabled, two in light mode) with the uptime sensor enabled and the Uptime integration added. v1.0.8-beta logged twelve-plus clean hours before the v1.0.9-beta structural changes superseded it.
...
| Version | Notes |
| --- | --- |
| 1.0.9-beta | Two structural changes. (1) HA-start block gate template rewritten to handle timestamp-mode `sensor.uptime`. The v1.0.6-beta template did `states(uptime_sensor) | float(99999)` and a `unit_of_measurement` lookup, which worked only when the Uptime integration was in numeric duration mode. The modern config-flow Uptime integration in HA 2026.6+ produces only timestamp mode (an ISO8601 datetime like `2026-06-16T08:32:18+00:00`), where `float()` failed and fell back to the sentinel `99999`. The comparison `99999 < ha_start_block_seconds` was always false, so the gate was silently inert across all v1.0.6-beta through v1.0.8-beta deployments. New template computes `(now() - as_datetime(states(uptime_sensor))).total_seconds()`. The post-restart branch was unaffected because it uses `ha_start_block_seconds` directly as a `delay:`, not through the gate. The bug went undetected partly because v1.0.7-beta's switch from queued to single mode absorbed the false-positive triggers the gate would otherwise have suppressed. (2) Added `max_exceeded: silent` alongside `mode: single`. v1.0.7-beta switched to single mode, which silently drops any trigger that arrives while a run is in progress; HA's default is to log a warning-level "Already running" message on each drop. The drops are correct behavior (a paired-entity duplicate trigger should not start a second cycle), but the warnings were log noise. `max_exceeded: silent` suppresses them. No behavior change beyond the log output. README updated: "A Small Note on the HA Start Block Window" section documents the bug and fix; `uptime_sensor` parameter table entry rewritten for timestamp mode; Beta Status narrative refreshed. **BREAKING for v1.0.6-beta through v1.0.8-beta users:** if you enabled `use_uptime_sensor` thinking the block was protecting boot, it was not. v1.0.9-beta makes the block actually engage. Users who had the block enabled and have not been seeing boot-window false positives can disable it again if they prefer the same effective behavior; users who want the boot-window protection should leave it enabled. |
