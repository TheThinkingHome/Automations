...
**Verified in production (v1.0.7-beta):** pending. The author's home test run starts on three FP2 instances (one in responsive mode with the active window enabled, two in light mode) with the uptime sensor enabled and the Uptime integration added.
