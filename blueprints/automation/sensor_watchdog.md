...
The block window addresses this by reading from the built-in HA Uptime integration. Add the integration via Settings > Devices & Services > Add Integration > Uptime; it produces a sensor (`sensor.uptime` by default) whose state is an ISO8601 datetime marking the moment HA last started. The blueprint computes elapsed seconds via `(now() - sensor_value).total_seconds()` and compares against the configured `ha_start_block_seconds`. While elapsed seconds is below the threshold, the stale, unavailable, and scheduled recovery branches exit silently. Once it crosses the threshold, the gate opens and the blueprint operates normally.

The block is opt-in. If `use_uptime_sensor` is unchecked or the configured sensor is unavailable, the gate is bypassed and the stale, unavailable, and scheduled branches rely on their per-trigger `for:` delays to ride out brief boot-window flicker. The per-trigger `for:` delays are usually enough; the Uptime-based block is belt and suspenders for installs with a lot of slow-loading integrations.
...
| Version | Notes |
| --- | --- |
| 1.0.0 | Initial stable release. Three independent failure detectors fire into a single recovery cycle: an offline detector (`unavailable` or `unknown`, optionally including `off` for ping-style sensors) with a tolerance window; a freshness detector backed by a timer helper that catches frozen-but-still-online devices; an optional daily scheduled cycle for preventive maintenance. A self-recovery branch turns the recovery switch back on if it is externally toggled off, using a `for:` delay so the recovery cycle's own turn-off does not fight itself. A post-restart evaluation branch fires on Home Assistant start, waits for integrations to load, then cycles the switch if any monitored entity is already in a failure state. Two detection modes (`light` and `responsive`) select how much event traffic the freshness detector creates; light is the default and needs no timer helper. An optional active time window gates the freshness branch only, leaving the offline detector and the scheduled cycle to fire any time of day. An optional uptime-sensor-based block (built on Home Assistant's built-in Uptime integration) suppresses the offline, stale, and scheduled branches while HA is still booting. An optional failure-notification path waits a configurable settle period after each cycle, checks whether any monitored entity is still failing, and runs a user-supplied action sequence if so; `recovery_reason` and `failed_entities` are exposed in the action context. Automation mode is `single` with `max_exceeded: silent`, so paired-entity simultaneous triggers do not duplicate cycles and the drop is silent in the log. Verified across 46 simulator scenarios and a four-instance home deployment (three Aqara FP2 presence sensors plus one router via the built-in ping integration). Beta-cycle history (1.0.0-beta through 1.0.9-beta) archived in the project document. |
