...
It does not poll. It re-evaluates the moment a watched Sentinel sensor changes, when a source has been unavailable long enough to be declared broken, once when Home Assistant starts, and again the moment a source finishes settling after a restart, to reconcile any recovery that became visible while the run was held. There is no check interval to tune. The only scheduled event is the optional daily reminder, described below.
...
Holding a run has one consequence to close: a recovery can happen *while* a source is settling (a low battery replaced, or a threshold changed, during the grace after a reboot), and that held run cannot act on it. If the source then holds steady, no ordinary state change would ever fire to reconcile it, and the stale item could sit on the list until the next daily reminder. So a dedicated trigger wakes the companion the moment a source finishes settling (its `settled` attribute turning true), and that run reconciles the list against current truth and removes anything that recovered during the hold. It carries no reminder or all-clear of its own, it only reconciles, so it is silent when there is nothing to sweep. With several sources it fires per source as each settles, and the settle gate holds every fire until the last source is ready, so the early ones are harmless. The one remaining deliberate trade: a recovery that arrives as a source's return from unavailable is acted on one evaluation later than before.

One final guard makes the run itself atomic against reloads. A template reload can swap a source entity in the milliseconds between the gates passing and the diff reading the data, so the diff would consume a flagged list captured during the reload's empty instant and delete every item as recovered. So the flagged list is re-read at the diff point, in the same step as a fresh blip-and-settle check: a source that swapped to an untrustworthy state holds the run (a diff-read hold in the log), and a swap after that point is harmless because the diff then uses the already-validated read. The gates and the diff always see one world.
...
| Version | Notes |
| --- | --- |
| 2.0.0-alpha.12 | One schema, two failure types. Reads the `kind` field the 1.2.0/2.4.0 Sentinels publish. Item lines mark an entity fault on a living device as `entity fault:`, and every line carries a stable `since <day> <time>` from the problem's start, fixed per episode so item text never churns. Push copy lists device-down lines before entity faults. Entries without the field render exactly as before. |
| 2.0.0-alpha.11 | The diff-read snapshot, from a live incident: a template reload during a deploy swapped a source entity mid-run. The gates passed honestly on the pre-swap entity, but the diff consumed the trigger-time flagged list, captured during the reload's empty instant, and computed two recoveries, silently deleting an acknowledged item and an open one; a restart then re-added both unchecked. Fix: a new step just before the to-do read re-reads the flagged list (shadowing the trigger-time values) and re-evaluates the blip and settle-stale predicates on that same read; if either holds, the run stops with a diff-read hold. A swap before the step is caught, a swap after it leaves the diff on the already-validated read, so the gates and the diff always see one world. No input change. |
| 2.0.0-alpha.10 | The settle-reconcile trigger. Fixes a stranded-recovery bug found live: a source recovered (its flagged item cleared) while still inside its startup grace after a reboot, so the run was correctly held and the recovery was not processed; the source then held steady, no state change fired, and with the daily reminder already sent, nothing woke the companion to reconcile until the next day. The item sat on the to-do list wrongly for up to a day, and a manual refresh could not clear it (the source state was unchanged, so the press produced no delta). Fix: a new per-source trigger fires when a source's `settled` attribute turns true (the moment it leaves grace), and that run reconciles the list, sweeping any recovery that became visible during the hold. It carries no reminder or all-clear, so it is silent when there is nothing to sweep, and with several sources the settle gate holds each per-source fire until the last source is ready. The 9.3 stale-source guard is unaffected. No input change. |
| 2.0.0-alpha.9.4 | Debug logging only; no engine or input change. The result summary log line now names the entities in its list-mutating fields, not just their counts: `additions`, `kind_changed`, `silent_updates`, and `recovered` each print a bracketed `[entity_id, ...]` list when non-empty (nothing when zero, so idle runs read as before). Prompted by a live incident where a frozen entity was flagged overnight and silently recovered at the next scan: the recovery was logged, but as `recovered: 1` with no name, so identifying which entity left the list meant cross-referencing the source sensor's own log. The notifier's stream is now self-documenting for every list change. |
