...
| Version | Notes |
| --- | --- |
| 1.0.0-alpha.7.2 | Five hardening fixes from an external code review plus an adversarial simulation pass, all non-breaking, no input changes. (1) A source that dies mid-day is now reported the moment the debounce elapses: a new trigger wakes the automation when a source has been unavailable for the full debounce, instead of the alert waiting for the next unrelated event, up to a day later. It cannot fire during a normal restart, which recovers inside the window. (2) A brief source blip (a Z2M restart, a template reload) no longer wipes the change-detection memory or re-announces known items on recovery: the run is held, like the restart hold, while a source is unavailable inside the debounce. Known limit: an outage longer than the debounce still re-announces that source's items when it recovers. (3) The optional refresh button is only pressed for a source unavailable inside the debounce, a recovery candidate. A source already declared broken is never pressed or waited for again, so one dead Sentinel no longer delays every alert by the refresh timeout. (4) Deleting the memory helper after setup now stops with a clear setup error instead of silently breaking change detection and re-notifying known items; a freshly created helper remains valid. (5) Fixed the overnight reminder mode, broken since 7.1: Home Assistant renders the stored held flag as a number, a comparison expected text, and the overnight delivery silently never fired. If you ran 7.1 in overnight mode, this is why the morning report never arrived. |
