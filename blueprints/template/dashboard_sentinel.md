...
A dashboard for your Sentinels: a status board that shows what has gone quiet and what is running low across your whole house, and the aggregator sensor that makes a combined board possible.

This page builds the board below in three steps: first the **aggregator**, the sensor that merges your Entity Sentinels into one clean list; then the **status cards** that read it; then, if you run Sentinel Notify, the **to-do cards** that make the board interactive. The full copy-paste view closes the page.
...
The aggregator comes first because the cards read it. If your setup does not need one (see above), skip to Step 2 and point the Entities card at your single Entity Sentinel instead.
...
Restart once (a brand-new package file registers at startup), and `sensor.dashboard_sentinel` appears, carrying the combined list the cards below will read.
...
## Step 2: The Status Cards

The board's reporting layer, built from stock `markdown` cards so it is portable. Each card shows a calm "all healthy" state when there is nothing to report and a formatted list when there is; the screenshot at the top of the page shows both states at once.

The four cards are shown one at a time below. The responsive arrangement, the header and footer spanning the full width with the two columns side by side on a wide screen and stacked on a phone, is done with [layout-card](https://github.com/thomasloven/lovelace-layout-card), the one HACS helper this board depends on. In a `custom:grid-layout` view each card also carries a `view_layout` line naming its grid area (`header`, `battery`, `entity`, `battery-list`, `entity-list`, `footer`); drop a card into an ordinary dashboard without layout-card and it still renders, it simply stacks in normal order. The card content below is what matters; the layout is yours to arrange.

The Batteries column reads `sensor.battery_sentinel`. The Entities column reads the aggregator you just built (`sensor.dashboard_sentinel`), or a single Entity Sentinel if you skipped Step 1. The `devices` shape is the same either way, so the same card works whichever you feed it.
...
cards:
  ...
  - type: markdown
    content: |
      <div style="text-align: center; opacity: 0.6; font-size: 0.85em;">Battery Sentinel + Dashboard Sentinel &nbsp;·&nbsp; updates live &nbsp;·&nbsp; check an item to acknowledge it</div>
```

## Step 3: The To-do Cards, If You Run Sentinel Notify

The status cards report; the to-do cards are where you act. If you run [Sentinel Notify](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/automation/sentinel_notify.md), each of its copies maintains a Local To-do list, one item per flagged device, and Home Assistant's standard to-do list card puts that list on the board, with the checkbox as the acknowledgment control. In the screenshot at the top, these are the Battery Shopping List and the Entity Outage Board, and the checked truck sensor is an acknowledged item. Add one card per list:

```yaml
type: todo-list
entity: todo.battery_sentinel
title: 🔋 Battery Shopping List
hide_create: true
```

```yaml
type: todo-list
entity: todo.entity_sentinel
title: 📡 Entity Outage Board
hide_create: true
```

Three notes from running these daily. `hide_create: true` removes the add-item box; manual items are harmless (the engine ignores anything it did not create), but the box invites clutter on what should read as a report. Leave completed items visible, which is the card's default, because that section is where acknowledged entries live, and hiding it hides the acknowledgment workflow. And avoid the card's overflow menu action "Remove completed items": it hand-deletes acknowledgments, and the engine re-adds and re-notifies anything still down. The full semantics, what checking, unchecking, and deleting each do, are in the [Sentinel Notify README](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/automation/sentinel_notify.md).

If you do not run Sentinel Notify, skip this step; the board above is complete without it.

## The Complete View

My exact dashboard, the one in the screenshot, as one paste-ready view: raw configuration editor, or add a view in YAML mode, and paste. It uses [layout-card](https://github.com/thomasloven/lovelace-layout-card) for the responsive grid, and it includes the two to-do cards; if you do not run Sentinel Notify, delete the two `todo-list` cards and the `battery-list` and `entity-list` lines from the two `grid-template-areas` blocks.
    ...
    cards:
      - type: markdown
        ...
        content: >
          ...
          ### {{ b }} {{ 'battery' if b == 1 else 'batteries' }}
          low&nbsp;&nbsp;·&nbsp;&nbsp;{{ e }} {{ 'entity' if e == 1 else
          'entities' }} quiet
      ...
      - type: markdown
        ...
        content: >
          ...
          {{ d.level }}%{% if d.battery_type %} · {{ d.battery_type }}{% endif
          %} · *{{ d.area }}*
          ...
          <ha-icon icon="mdi:check-circle" style="color: var(--success-color,
          #4caf50); --mdc-icon-size: 32px;"></ha-icon>
          ...
          <div style="opacity: 0.55; font-size: 0.8em; margin-top: 8px;">{% if t
          and t not in ['unknown', 'unavailable'] %}Last checked {{ (t |
          as_datetime | as_local).strftime('%-I:%M:%S %p') }}{% else %}Last
          checked: unknown{% endif %}</div>
      - type: markdown
        view_layout:
          grid-area: entity
        content: >
          ...
          {{ d.reason }}{% if d.age and d.age != 'unknown' %} · {{ d.age }}{%
          endif %} · *{{ d.area }}*
      ...
      - type: vertical-stack
        ...
        cards:
          ...
          - type: markdown
            content: >
              <div style="text-align: center; opacity: 0.6; font-size:
              0.85em;">Battery Sentinel + Dashboard Sentinel &nbsp;·&nbsp;
              updates live &nbsp;·&nbsp; check an item to acknowledge it</div>
```
