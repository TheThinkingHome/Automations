...
Entity Sentinel comes from **The Thinking Home** at [xeazy.com](https://xeazy.com). The full design story and worked examples are in the [article](https://xeazy.com/battery-entity-sentinel-blueprints/). It has a companion: [Sentinel Notify](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/automation/sentinel_notify.md) reads this sensor and turns it into change-aware phone notifications and a live to-do list you check off as you fix things. Entity Sentinel detects; Sentinel Notify tells you. They are built to work together, and most installations will want both.
...
**Two levels of judgment.** `unavailable`, `unknown`, `missing`, and `never_reported` are read straight from the entity you named. `frozen` is judged at the **device** level: the freshest report from any entity on the same device counts, so a quiet entity whose sibling is still chatting is never flagged, the device is demonstrably alive. A motion sensor is the classic case: its motion entity may be silent for hours in an empty room while its signal strength updates constantly; the device is fine, and Entity Sentinel knows it.
...
Two notes worth knowing. A tier's target takes a list, so one tier can watch several labels on one window, the sample's Sleepy tier watches `battery_watch` too, so battery entities gone silent are caught on the same window. And adding a device to the watch later is just applying the label to it, no package edit, no restart.
