...
A Home Assistant template blueprint that catches entities gone quiet: unavailable, unknown, missing, or frozen at their last value while the device behind them has silently died. One sensor, up to five tiers, each tier with its own targets and its own freeze window, evaluated together on one cadence and published as one merged, tier-annotated list.
...
The 1.x Entity Sentinel had one window per sensor: you told it how long silence was allowed to last, and it watched everything you gave it against that single clock. But a motion sensor that reports every few minutes and a leak sensor that speaks once a day do not share a definition of "too quiet." The 1.x answer was one sensor per reporting rhythm, a Lively copy at 8 hours, a Quiet copy at 16, a Sleepy copy at 36, and that answer worked, but it had a repercussion: three sensors meant three separate flagged lists, and making them useful, on a dashboard card, in a notifier, as one number, took an extra aggregation step. That step was [Dashboard Sentinel](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/template/dashboard_sentinel.md), a blueprint whose sole reason to exist was merging the copies back into one list.

2.0 moves the aggregation inside the detector. The tiers live in the sensor: each tier carries its own targets and its own freeze window, one evaluation judges every entity against its own tier's clock, and the output is one merged list, the same shape the aggregator used to produce. One package block replaces three blocks plus an aggregator. The configuration changes shape slightly, the window inputs become per-tier inputs, and the result is one sensor doing what two blueprints did together.

This is a breaking change in configuration, not in output. The output contract is a strict superset of 1.x, so Sentinel Notify and existing dashboard cards consume the new sensor unchanged. But a 1.x package block does not carry over as written, which is why the version is 2.0 and why the 1.x line is preserved: **the 1.x blueprint and its README remain downloadable, frozen at 1.0.5-beta**, as [entity_sentinel_1.0.5-beta.yaml](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/template/entity_sentinel_1.0.5-beta.yaml) and [entity_sentinel_1.0.5-beta.md](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/template/entity_sentinel_1.0.5-beta.md). Existing 1.x sensors keep working forever; they simply receive no further development. Dashboard Sentinel likewise remains available for 1.x setups and is superseded for new builds.

**Current version: 2.0.0-beta.** The 2.0 engine graduated through a 57-check adversarial simulation suite, two external code reviews (one of which found a real counting bug that was fixed before this release), and live duty on the author's own system: a week of nightly reboot cycles, real detections including a frozen door sensor caught at exactly its tier's window, and a full cutover replacing the three-sensor-plus-aggregator pattern it supersedes. Beta means the engine is settled and the remaining risk is variety: more homes, more integrations, more hardware weirdness. If you hit something, the [community thread](https://xeazy.com/logbook/d/42-the-battery-entity-sentinel-blueprints) is where it gets found and fixed.
