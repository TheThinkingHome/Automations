# Entity Sentinel (1.0.5-beta, frozen)

> **This line is frozen.** Entity Sentinel 2.0 supersedes it for new builds: one sensor with up to five tiers, each with its own freeze window, replacing the one-sensor-per-window pattern and the Dashboard Sentinel aggregator. Existing 1.x sensors keep working and this file stays downloadable, but the line receives no further development. See the current [Entity Sentinel README](https://github.com/TheThinkingHome/Automations/blob/main/blueprints/template/entity_sentinel.md).
...
| Version | Notes |
| --- | --- |
| 1.0.5-beta | Added two identity attributes: sentinel_type (entity) and sentinel_version (the running version string). They let a dashboard, a companion, or a bug report identify the sensor and its version without opening the blueprint, and they are present even in the setup_error state. No detection change. |
