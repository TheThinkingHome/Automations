...
| `backup_weights` | No | blank | comma-separated numbers, one per backup, in the same order | Weighted average when filled, equal weighting when blank. Weight zero means the sensor contributes nothing to the weighted average while better-weighted sensors are online, and is used as a last-resort backup only when every weighted sensor is offline. More than one weight-zero sensor is allowed; if several are the only ones reporting, they are averaged equally. If the count of weights does not match the count of backups, any weight is not a number, or every weight is zero, the blueprint falls back to equal weighting. |
...
| Version | Notes |
| --- | --- |
| 1.0.2 | Adds a last-resort fallback for the case where every backup currently reporting has weight zero. Previously, weights like `"100, 0"` with the weight-100 sensor offline would collapse `total_weight` to zero and fall through to `default_value` or `unavailable`. Now the blueprint detects that situation, drops the weights, and equal-averages whichever backups are still reporting. The user-facing effect: weight zero now means "contributes nothing while better-weighted sensors are alive, and takes over only when every weighted sensor is offline." Multiple weight-zero sensors are allowed and are equal-averaged when they are the only ones reporting. Lets users stage a last-resort tier of sensors without contaminating the weighted result. The parameter description and parameters table in the README are updated to document the semantics. |
| 1.0.1 | Handles the all-zero-weights configuration case. If every backup weight is set to zero (a user-config mistake or the entire intent of the field), the blueprint now falls back to equal weighting at config-time instead of letting `total_weight` collapse to zero and dumping the user to `default_value` or `unavailable`. |
| 1.0.0 | Initial release. Wraps a numeric sensor with a pool of backups so a single flaky source no longer surfaces as `unavailable` to downstream automations. Primary's reading is returned when available. When the primary is offline, online backups are averaged, optionally weighted. When everything is offline, an optional default value is returned or the sensor reports `unavailable`. Exposes `active_source` and `online_backups` attributes for transparency. |
